...
const title = computed(() => String(maintenanceCopy.value.title ?? "We'll be right back"))
const lead = computed(() =>
  String(maintenanceCopy.value.lead ?? 'The readathon site is briefly down for maintenance.'),
)
const body = computed(() =>
  String(
  ...
  <main class="page maintenance-page">
    <div v-if="config" class="maintenance-card card">
      ...
      <p v-else class="fine-print">
        ...
        <RouterLink to="/login">{{ maintenanceCopy.loginLink ?? 'Sign in' }}</RouterLink>
      </p>
    </div>
  </main>
</template>
