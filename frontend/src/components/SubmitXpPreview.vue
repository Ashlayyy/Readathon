...
const yourTeamGain = computed(() => {
  ...
  if (props.submissionType === 'add') return promptAndBonus.value + props.pageBonus
...
const lines = computed((): XpLine[] => {
  ...
  if (props.pageBonus > 0) {
    rows.push({
      label: props.submissionType ? 'Page bonus (your realm)' : 'Page bonus for your realm',
...
<template>
  <aside v-if="submissionType || pageBonus > 0" class="xp-preview-panel" :class="{ compact }">
    <header class="xp-preview-header">
      ...
      <h3 class="xp-preview-title">
        ...
        <template v-else-if="submissionType === 'add'">Adding to your realm</template>
    ...
    <ul v-if="lines.length" class="xp-lines">
      <li v-for="(line, i) in lines" :key="i" class="xp-line">
        ...
        <span class="xp-line-value" :class="line.tone">{{ formatSigned(line.value) }} XP</span>
      </li>
    </ul>
    ...
    <div v-else-if="pageBonus > 0" class="xp-totals">
      <div class="xp-total-card gain">
        <span class="xp-total-label">{{ userTeam?.name ?? 'Your realm' }}</span>
        <strong class="xp-total-value">+{{ pageBonus }} XP</strong>
        <span class="xp-total-sub">From page count — stacks with prompts later</span>
      </div>
    </div>
  </aside>
</template>
...
.xp-preview-panel {
  ...
  padding: 1rem 1.1rem;
  border-radius: 14px;
  border: 1px solid color-mix(in srgb, var(--realm-accent) 28%, var(--realm-border));
  background:
