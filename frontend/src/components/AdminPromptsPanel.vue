...
const filtered = computed(() => {
	return prompts.value.filter((p) => {
		...
		if (q) {
			const haystack =
				`${p.label} ${p.description} ${p.promptId} ${p.gameName} ${p.kind} ${p.teamId ?? ''}`.toLowerCase();
...
const grouped = computed(() => {
	const sortByUnlock = (items: AdminPrompt[]) =>
		[...items].sort((a, b) => {
			const ta = a.goesLiveAt
				? new Date(a.goesLiveAt).getTime()
				: Number.MAX_SAFE_INTEGER;
			const tb = b.goesLiveAt
				? new Date(b.goesLiveAt).getTime()
				: Number.MAX_SAFE_INTEGER;
	...
	const positive = sortByUnlock(
		filtered.value.filter((p) => p.kind === 'positive'),
	);
	const negative = sortByUnlock(
		filtered.value.filter((p) => p.kind === 'negative'),
	);
	const teamBonus = sortByUnlock(
		filtered.value.filter((p) => p.kind === 'team_bonus'),
	);
...
const promptSections = computed(() => [
	{
		key: 'positive',
		title: section('prompts').groupPositive,
		items: grouped.value.positive,
	},
	{
		key: 'negative',
		title: section('prompts').groupNegative,
		items: grouped.value.negative,
	},
	{
		key: 'teamBonus',
		title: section('prompts').groupTeamBonus,
		items: grouped.value.teamBonus,
	},
]);
...
function isScheduled(p: AdminPrompt) {
	return Boolean(
		p.isActive && p.goesLiveAt && new Date(p.goesLiveAt) > new Date(),
	);
}
...
async function onUploadFileSelected(event: Event) {
	...
	try {
		...
		uploadError.value =
			section('prompts').uploadInvalidJson ?? 'Invalid JSON file.';
		uploadFile.value = null;
		uploadPack.value = null;
	}
}

function previewPackClient(data: unknown): {
	total: number;
	scheduled: number;
} {
	const pack = data as {
	...
	if (Array.isArray(pack.prompts)) {
		...
		for (const p of [
			...(pack.prompts.positive ?? []),
			...(pack.prompts.negative ?? []),
		] as {
			goesLiveAt?: string;
		}[]) {
			countPrompt(p);
	...
	for (const team of pack.teams ?? []) {
		for (const p of (team.bonusPrompts ?? []) as { goesLiveAt?: string }[])
			countPrompt(p);
	}
...
async function runJsonUpload() {
	...
	try {
		const result = await api<{ imported: number; scheduled: number }>(
			'/admin/prompts/import-json',
			{
				method: 'POST',
				body: JSON.stringify({
					replaceExisting: importReplace.value,
					pack: uploadPack.value,
				}),
			},
		);
		emit(
...
async function savePrompt() {
	...
	try {
		const body = {
			...
			goesLiveAt: form.value.goesLiveAt
				? new Date(`${form.value.goesLiveAt}T00:00:00.000Z`).toISOString()
				: null,
		...
		emit(
			'message',
			e instanceof Error ? e.message : msg('promptSaveFailed'),
			true,
		);
	} finally {
		loading.value = '';
...
async function removePrompt(p: AdminPrompt) {
	...
	try {
		...
		emit(
			'message',
			e instanceof Error ? e.message : msg('promptDeleteFailed'),
			true,
		);
	} finally {
		loading.value = '';
...
async function runImport() {
	...
	try {
		const result = await api<{ imported: number }>(
			'/admin/prompts/import-from-config',
			{
				method: 'POST',
...
<template>
	<section class="card admin-section prompts-panel">
		<div class="prompts-header">
			<div>
				...
				<p class="section-desc">
					...
					<span v-if="usingDatabase">{{
						t(section('prompts').statLive, { count: stats.liveCount })
					}}</span>
					<span v-else>{{ section('prompts').leadJson }}</span>
					<span v-if="stats.scheduledCount">
						·
						{{
							t(section('prompts').statScheduled, {
								count: stats.scheduledCount,
							})
						}}</span
					>
					<span v-if="stats.draftCount">
						·
						{{
							t(section('prompts').statDraft, { count: stats.draftCount })
						}}</span
					>
				</p>
			</div>
		...
		<div v-if="loading === 'load'" class="alert alert-info">
		...
		<template v-else>
			<div v-for="sec in promptSections" :key="sec.key" class="prompt-group">
				<h3>
					{{ sec.title }} <span class="count">({{ sec.items.length }})</span>
				</h3>
				<div v-if="sec.items.length === 0" class="empty-group">
					{{ section('prompts').emptyFilter }}
				</div>
					...
					>
						<div class="prompt-main">
							<div class="prompt-top">
								<span
									class="xp-badge"
									:class="p.points > 0 ? 'gain' : 'attack'"
								>
									{{ p.points > 0 ? '+' : '' }}{{ p.points }}
								</span>
								<div class="prompt-heading">
									<strong class="label">{{ p.label }}</strong>
									<span v-if="p.kind === 'team_bonus'" class="sub">{{
										teamName(p.teamId)
									}}</span>
									<span v-else-if="p.gameName" class="sub">{{
										p.gameName
									}}</span>
								</div>
								<span class="status-pill" :class="statusClass(p)">{{
									statusLabel(p)
								}}</span>
							</div>
							<p v-if="p.description" class="desc">{{ p.description }}</p>
							<div class="prompt-meta">
								<span v-if="isScheduled(p)" class="schedule-pill"
									>Unlocks {{ formatGoLiveDate(p.goesLiveAt) }}</span
								>
			...
			<div class="modal card prompt-modal">
				<header class="modal-header">
					<h2>
						{{
							createMode
								? section('prompts').addTitle
								: section('prompts').editTitle
						}}
					</h2>
				</header>
				<form class="prompt-form modal-body" @submit.prevent="savePrompt">
					...
					<label>
						...
						<select v-model="form.kind">
							<option value="positive">
								{{ section('prompts').kindPositive }}
							</option>
							<option value="negative">
								{{ section('prompts').kindNegative }}
							</option>
							<option value="team_bonus">
								{{ section('prompts').kindTeamBonus }}
							</option>
						</select>
					</label>
					...
					<label v-if="form.kind !== 'team_bonus'">
						...
						<input
							v-model="form.link"
							type="url"
							:placeholder="section('prompts').linkPlaceholder"
						/>
					</label>
					...
					<div class="modal-actions">
						...
						>
							{{
								loading === 'save'
									? section('prompts').saving
									: section('prompts').save
							}}
						</button>
					</div>
				</form>
			...
			<div class="modal card prompt-modal add-menu-modal">
				...
				<div class="modal-body">
					...
					<div class="add-menu-options">
						...
						<button
							type="button"
							class="add-menu-option subtle"
							@click="chooseConfigImport"
						>
					...
					<div class="modal-actions">
						<button type="button" class="btn btn-ghost" @click="closeAddMenu">
							{{ section('prompts').cancel }}
						</button>
					</div>
				</div>
				...
				<div class="modal-body">
					...
					<div class="file-upload">
						...
						<span class="file-name">{{
							uploadFile?.name ?? section('prompts').uploadNoFile
						}}</span>
					</div>
					...
					<p v-else-if="uploadPreview" class="upload-preview">
						{{
							t(section('prompts').uploadPreview, {
								count: uploadPreview.total,
								scheduled: uploadPreview.scheduled,
							})
						}}
					</p>
					...
					<div class="modal-actions">
						...
						>
							{{
								loading === 'import'
									? section('prompts').uploadImporting
									: section('prompts').uploadImport
							}}
						</button>
					</div>
				</div>
				...
				<div class="modal-body">
					...
					<div class="modal-actions">
						...
						>
							{{
								loading === 'import'
									? section('prompts').importing
									: section('prompts').importSubmit
							}}
						</button>
					</div>
				</div>
