...
import { validateSubmission, type SubmissionInput } from './services/scoring.js'
...
describe('security manual checks', () => {
  before(async () => {
    if (!process.env.MONGODB_URI) {
      throw new Error('MONGODB_URI is required. Set it in server/.env to run security checks.')
    }
    await connectDb()
  ...
  it('1) rejects submission that uses a draft (inactive) prompt ID', async () => {
    ...
    assert.equal(draftError, 'Invalid prompt selection.', `expected draft rejection, got: ${draftError}`)
  })
  ...
  it('3) register does not create a session before magic-link verify', async () => {
    ...
    const registerBody = (await registerRes.json()) as { sent: boolean; message: string }
    ...
    assert.equal(registerCookie, null, 'register must not set realm_session cookie')

    const meRes = await app.request('/api/auth/me')
    const meBody = (await meRes.json()) as { user: unknown }
    assert.equal(meBody.user, null, '/auth/me should be null without verified session')
  })
})
