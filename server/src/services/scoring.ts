...
export async function validateSubmission(
  ...
  if (input.submissionType === 'sabotage') {
    ...
    if (input.targetTeamId === user.teamId) return 'You cannot attack your own team.'
...
export function calculateScore(user: HydratedDocument<IUser>, input: SubmissionInput): ScoreBreakdown {
  ...
  for (const id of input.bonusTeamPromptIds) {
    ...
    if (tp) {
      bonusDetails.push({ id: tp.id, label: tp.label, points: tp.points * sign })
    }
  }
...
export async function calculateStandings(): Promise<TeamStanding[]> {
  ...
  for (const sub of allSubs) {
    ...
    if (sub.submissionType === 'add') {
      gained.set(teamId, (gained.get(teamId) ?? 0) + promptAndBonus + sub.pageBonus)
